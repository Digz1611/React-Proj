...
const SupportTicket = () => {
  ...
  const handleCreateTicket = async (e) => {
    ...
    try {
      await axios.post('https://us-central1-cinemate-4d8fe.cloudfunctions.net/api/supportTicket', {
        email: user.email,
        subject,
        message,
        userId: user.uid,
  ...
  const handleReply = async () => {
    ...
    try {
      await axios.post(`https://us-central1-cinemate-4d8fe.cloudfunctions.net/api/supportTicket/${currentTicket.id}/respond`, {
        reply,
        isAdmin,
        uid: user.uid
      });
