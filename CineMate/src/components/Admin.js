...
const Admin = () => {
  ...
  const handleDeleteUser = async (userId) => {
    try {
      ...
      await axios.delete(`https://us-central1-cinemate-4d8fe.cloudfunctions.net/api/user/${userId}`);
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        if (userAnswer === userDoc.data().securityAnswer) {
          ...
          const response = await axios.post('https://us-central1-cinemate-4d8fe.cloudfunctions.net/api/resetPassword', {
  ...
  const handleReply = async () => {
    if (currentTicket && reply) {
      const response = await axios.post(`https://us-central1-cinemate-4d8fe.cloudfunctions.net/api/supportTicket/${currentTicket.uniqueIdentifier}/respond`, { reply });
      if (response.status === 200) {
  ...
  const handleDeleteTicket = async (ticketId) => {
    try {
      const response = await axios.delete(`https://us-central1-cinemate-4d8fe.cloudfunctions.net/api/supportTicket/${ticketId}`);
      if (response.status === 200) {
