...
const Admin = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const itinerarysSnapshot = await getDocs(collection(firestore, 'itinerarys'));
        const supportTicketsSnapshot = await getDocs(collection(firestore, 'supportTickets'));

        const usersList = usersSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        const itinerarysList = itinerarysSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        const supportTicketsList = supportTicketsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));

        setUsers(usersList);
        setItinerarys(itinerarysList);
  ...
  const handleDeleteItinerary= async (itineraryId) => {
    try {
      const firestore = getFirestore();
      await deleteDoc(doc(firestore, 'itinerarys', itineraryId));
  ...
  const handleDeleteUser = async (userId) => {
    try {
      ...
      await axios.delete(`https://us-central1-expeditionease-95ada.cloudfunctions.net/api/user/${userId}`);
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        if (userAnswer === userDoc.data().securityAnswer) {
          ...
          const response = await axios.post('https://us-central1-expeditionease-95ada.cloudfunctions.net/api/resetPassword', {
  ...
  const handleReply = async () => {
    if (currentTicket && reply) {
      const response = await axios.post(`https://us-central1-expeditionease-95ada.cloudfunctions.net/api/supportTicket/${currentTicket.uniqueIdentifier}/respond`, { reply });
      if (response.status === 200) {
  ...
  const handleDeleteTicket = async (ticketId) => {
    try {
      const response = await axios.delete(`https://us-central1-expeditionease-95ada.cloudfunctions.net/api/supportTicket/${ticketId}`);
      if (response.status === 200) {
        ...
        <tbody>
          {itinerarys.map(itinerary=> (
            <tr key={itinerary.id}>
              ...
              <td>{itinerary.createdBy}</td>
              <td>{itinerary.status}</td>
              <td>
                <Button variant="danger" onClick={() => handleDeleteItinerary(itinerary.id)}>Delete</Button>
              </td>
            </tr>
          ))}
        </tbody>
